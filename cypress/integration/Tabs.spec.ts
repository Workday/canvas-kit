...
describe('Tabs', () => {
  ...
  context(`given the 'Simple' story is rendered`, () => {
    beforeEach(() => {
      h.stories.load('Testing/React/Labs/Tabs', 'Simple');
    });
      ...
      context('when the right arrow key is pressed', () => {
        ...
        context('when the space key is pressed', () => {
          ...
          it('should have "aria-selected=true" on the second tab', () => {
            cy.findByRole('tab', {name: 'Second Tab'}).should('have.attr', 'aria-selected', 'true');
          });
        });
