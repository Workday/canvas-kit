...
Set the `mode` prop to `dialog` to convey the presence of a follow-up action. You can also use the
`useTostModel` to pass in a uniqueId:
...
> **Note**: You must use `Toast.Message` subcomponent when the mode is `dialog` so that `id` is tied
> to the message for accessibility
...
Use `Toast.CloseIcon` to provide a close button to dismiss the `Toast`. Use it with `Popup` and
