...
`SearchForm` is a search form that contains a combobox for rendering search results. It's primarily
intended to be used within a header.
...
If you'd like the `SearchForm` to grow the the width of the container, set the `grow` prop to
`true`.
...
The `SearchForm` can be themed to use one of preset `SearchThemes`: `Light`, `Dark`, and
