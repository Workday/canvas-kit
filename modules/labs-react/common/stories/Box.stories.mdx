...
// examples
import {AsPropLink} from './examples/As/Link';
import {AsPropCustomComponent} from './examples/As/CustomComponent';
...
const Card = (props: CardProps) => {
  return (
    <Box borderRadius="m" depth={2} padding="m" {...props} />
  );
}
```
...
another component. By default, `Box` will render a `div` element, but often that's not what you
...
<ExampleCodeBlock code={AsPropLink} />

---

<ExampleCodeBlock code={AsPropCustomComponent} />
...
use Canvas values as you build UI. There are five types of border props:
...
when you need bidrectional styles.
...
props, such as `left` and `right` when you need bidrectional styles.
