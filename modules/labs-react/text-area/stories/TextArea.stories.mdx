...
Use `TextArea.Field`'s `isRequired` prop (or use with the `useFormFieldModel` hook) to indicate that
...
extend full width, e.g. setting width prop to 100%, or you can wrap the whole `TextArea` in it a
Flex and set the growth that way, etc.

<ExampleCodeBlock code={Grow} />

### Label Position

Use `HStack` or `VStack` to set `TextArea.Label`'s position. Below are examples of both positions:
...
Use the `hasError` property from `useFormFieldModel` to set the `TextArea` to the Error state. If
you have an accompanying hint you can use the `TextArea.Hint` subcomponent.
