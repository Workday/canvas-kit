...
These will return [color-mix()](https://developer.mozilla.org/en-US/docs/Web/CSS/Reference/Values/color_value/color-mix) and the result is mixing the first color and the mixin color together in a given colorspace by a given amount.
...
1. `color`: The color that will be darkened (this is typically the "base" color on the given
   element).
2. `fallback`: A fallback color if the first color is not valid or not defined. This is optional, however, we recommend providing one.
