...
For v15, there is a **separated codemod** called `v15-icons` that updates Canvas Kit icons usage
across your codebase. This codemod will:

- For **system icons**:
  - Replace all **deprecated system icon** from `@workday/canvas-system-icons-web` with the correct
    new icon name.
...
**PR:** [#3732](https://github.com/Workday/canvas-kit/pull/3851)
...
- The `color` prop now only supports direct color values (tokens such as `blueberry400` are no
  longer accepted). Use the new token values directly instead.
...
For more information on our tokens, vie\w our
