...
Components are transitioning to [Compound Components](./COMPOUND_COMPONENTS.mdx):
...
taxonomy, we've renamed our `core` packages to `tokens`. We've also removed the `type` default
export in the labs core package in favor of named imports. These changes are listed below, all of
...
To better align with our design taxonomy, we've renamed our space tokens in our core package.
Instead of relying on `@workday/canvas-space-web` to supply our space values, we're now keeping
those values in canvas-kit. We've also taken the opportunity to improve the space types (which were
too generic) and JSDoc hints.
...
We've promoted several components in v5.

#### Promoted from Labs to Preview
...
// Or if you're already using the new slash imports syntax
import {Breadcrumbs} from '@workday/canvas-kit-labs-react/breadcrumbs';
...
Additionally, we've changed some of the Button module's export behavior:
...
- Buttons now use `createComponent` utility from the `common` module which now forwards `ref` and
  ...
  - Button prop interfaces no longer extend directly from
    ...
    uses a Button prop interface and accesses properties like `onClick`, you'll need to provide the
    button attribute yourself in order to avoid TypeScript issues (this doesn't affect runtime).
    This is not code-moddable since intent cannot be pre-determined.
...
Card is now a compound component composed of a `Card.Body` and an optional `Card.Heading`. This
allows direct access to the heading and body elements.
...
user's current input method) has been moved. After renaming our `core` package to `tokens`, it no
