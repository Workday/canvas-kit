...
replaces those with a new, responsive type hierarchy in `@workday/canvas-kit-react-core`. We are
...
All type hierarchy updates are handled by the codemod. The tables below will help you understand the
changes and provide a reference as you review your UI. Most teams are using the `beta` type tokens
in `@workday/canvas-kit-labs-react-core`, but some are using the `legacy` type in
`@workday/canvas-kit-labs-react-core`.

| Beta Type (px)  | Resposive Type (rem)                      |
| --------------- | ----------------------------------------- |
...
| Legacy Type (px)  | Resposive Type (rem)                     |
| ----------------- | ---------------------------------------- |
...
| `body2` (13px)    | `levels.subtext.small` (0.75rem \ 12px)  |
| `small` (12px)    | `levels.subtext.small` (0.75rem \ 12px)  |
...
| ------------ | ----------------------------- | --------------------------------------------------------------- |
| `fontFamily` | `type.properties.fonts`       | `normal` (Roboto) and `monospace` (Roboto Mono) are available   |
...
| `type.variant.button`  | `{fontWieight: type.properties.fontWeights.bold}`                              | variant deprecated, use type properties |
...
| `type.variant.mono`    | `{fontWeight: type.properties.fonts.monospace}`                                | variant deprecated, use type properties |
...
To migrate, please refer to the hierachy tables about and use the type hierarchy tokens directly. Detailed usage information is available in the [levels section](#levels).

##### Link Variant Migration

To migrate, please use the `Hyperlink` component instead. Detailed usage information is available in the [variants section](#variants).

#### Type Updates In-Depth Overview

The new type tokens introduce a few major changes:

- Introducing rem values
- Creating new `type` object structure
- Adding new type properties (`fonts`, `fontSizes`, and `fontWeights`)
- Updating and replacing variants

#### Introducing Rem Values

The new type hierarchy uses `rem` values instead of `px`. This update follows the guidance
...
hierarchy, and quite a few variants. While none of these are bad attributes, we found in our
research that they created a large amount of confusion. Both designers and engineers were unclear on
when to use many of the tokens provided. We restructured the object to help users make more sense of
it. The tokens are divided into three main categories:

- `levels` (the type hierarchy)
- `properties` (`fonts`, `fontSizes`, and `fontWeights`)
...
Instead of the previous flat structure, the type hierarchy is now organized in five levels:

- `display` (used in large heroes and images, not typically used in product applications)
...
And each level has three sizes: `small`, `medium`, and `large`. This organization allows the
hierarchy to be more flexible and provide better guidance for usage. Below is an example:

```tsx
// v4
import {type} from '@workday/canvas-kit-labs-react-core';

const headingStyles = type.h2;
const smallBodyStyles = type.body2;

// v5
import {type} from '@workday/canvas-kit-react/tokens';

const headingStyles = type.levels.heading.large;
const smallBodyStyles = type.levels.subtext.large;
```
...
particular value. Here's an example using `fonts`, `fontSizes`, and `fontWeights`.
...
There are also a handful of deprecated variants:
...
supported with `properties` and other styles. Below is an example of the translation neeed for each:
...
// mono variant styles
const monoStyles = {fontFamily: type.properties.fonts.monospace};
```

**Link Variant**

The `link` variant is also being deprecated in v5, and users will need to use the `Hyperlink`
component instead. This is the only manual update needed for the type updates. Below is an example:
...
return (
  <Link href="https://workday.github.io/canvs-kit">View docs</Link>
);
...
return (
  <Hyperlink href="https://workday.github.io/canvs-kit">View docs</Link>
);
```
...
This change is described in more detail in the [Type Section](#type), but suffice to say all `type`
imports will be automatically migrated to the Main `token` package by the codemod.
...
The `space` function was a handy little utility that you could apply to `styled()` components to add
space style props. However, with the addition of `Box` it is no longer needed. `Box` provides
`space` style props and much more. While this is a manual migration, the process is fairly
straight-forward. One important note: The `space` props were a shorthand version of what `Box`
provides: `pt` maps to `paddingTop`, `mr` maps to `marginRight`, and so on. You can see this in the
example below.
...
The codemod will handle _almost all of_ of these changes with a The v5 codemod handles aew possible
exceptions not. for you. That said, you'll want to review your UI to ensure everything was updated
as you expect. [Manual Updates](#manual-updates) below.
...
v5 introduced focus management behaviors like `useInitialFocus` and `useReturnFocus` that was
originally hard-coded into the `Modal`, but now `Modal` uses the hooks from the `popup` module. We
noticed some application code that do custom focus management. Most of the special focus management
code could be removed when using v5 the `Modal`.
