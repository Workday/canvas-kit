...
- Introducing
  [rem values](https://developer.mozilla.org/en-US/docs/Learn/CSS/Building_blocks/Values_and_units#ems_and_rems)
...
The `beta` and `legacy` type object structure was fairly flat, provided many levels in the type
hierarchy, and quite a few variants. While none of these are bad attributes, we found in our
research that they created a large amount of confusion. It was unclear for designers and engineers
...
> _Note:_ The `variant` key has been renamed to `variants` to be consistent with our other key
> names.
...
```tsx
const linkStyles = type.variant.link;
```
