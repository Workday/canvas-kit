...
these changes automatic updates.

**Renamed:**

- 🤖 `Button` has been split into `PrimaryButton` and `SecondaryButton` (depending on the `variant`
  prop).
- 🤖 `OutlineButton` (`secondary`) is now `SecondaryButton` with an `outline` variant.
...
Note: There may still be some confusion between the filled default and the `outline`
`SecondaryButton`. For accessibility reasons, the `outline` styling will become the new default and
the filled version will be removed in a future update. This is in progress, which is why it has been
pushed to v6.
