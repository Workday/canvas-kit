...
> Note: See [Canvas Kit Preview & Component Promotions](#canvas-kit-preview--component-promotions)
> for more information about the new `@workday/canvas-kit-preview-react` module.

Consequently, imports will need to be updated:
...
(`@workday/canvas-kit-labs-react`), we're introducing a new module called
...
We've promoted several components from Labs to Preview in v5:
...
introduced here in v5, however, we believe that a few components in Labs are ready for Main and thus
we've promoted them straight from Labs to Main:
...
taxonomy, we're renaming our `core` packages to `tokens`. We're also removing the `type` default
