...
> > > > > > > d1034754cce558ff9aa7b2c1d380fd05ac613236
...
`Popup` has transitioned to a compound component, along with all Popup-based behavior hooks. What
was a `Popup` prior to v5 is now a `Popup.Card` in v5. The target button and `Popper` components
have been moved to subcomponents of a `Popup`.
...
We noticed Popups were used in 2 different ways: always rendering and conditional rendering.
...
The difference between the two is subtle, but when always rendering example, the `usePopup` hook
...
cases, but `open` is not passed to the hook. This caused subtle bugs, and now `useCloseOnEscape` is
passed a `PopupModel` which does have access to the popup's visible state. `useCloseOnEscape` will
now only run when the popup is visible, but this means the conditional rendering example will have
to do extra work because the `target` is out of scope of the `MyOpenPopup` component. The following
