...
const MyCardHeading = styled('h2')({
  // styles copied from Canvas Card source code
  ...cardStyleOverrides
})

<Card>
  <MyCardHeading>Card Heading</MyCardHeading>
  Card Body
</Card>

// after using css
<Card>
  <Card.Heading as="h2" css={cardStyleOverrides}>Card Heading</Card.Heading>
  <Card.Body>Card Body</Card.Body>
</Card>

// after using styled
const MyCardHeading = styled(Card.Heading)(cardStyleOverrides)
<Card>
  <MyCardHeading as="h2">Card Heading</MyCardHeading>
  <Card.Body>Card Body</Card.Body>
</Card>
```
...
```tsx
// before
const {targetProps, popperProps, closePopup, stackRef} = usePopup();

useCloseOnOutsideClick(stackRef, closePopup);
useCloseOnEscape(stackRef, closePopup, buttonRef); // used to return focus to button

// after
const model = usePopupModel({
  returnFocusRef: buttonRef, // only needed if you don't use `Popper.Target`
});

const targetProps = usePopupTarget(model);
const popperProps = usePopupPopper(model);
const closePopup = model.events.hide;

// onClick that returns focus if not using `Popup.CloseButton` or `Popup.CloseIcon`
const onClick = () => {
  model.events.hide();

  // changeFocus comes from `common`
  changeFocus(buttonRef.current); // if `Popper.Target` was used, this could be `model.state.targetRef.current`
};

useCloseOnOutsideClick(model);
useCloseOnEscape(model);
useReturnFocus(model); // replaces the third parameter in `useCloseOnEscape`.
```
