...
Skeleton was already implemented as a compound component, but there are two notable changes: its
imports and the animation.

First, the imports for its sub-components have been updated to be keys on `Skeleton`, for Header,
Shape, and Text. You will now always import just `Skeleton` component, and you may still compose your
own Skeleton using whatever parts you need.

```tsx
// v4
import {Skeleton, SkeletonHeader, SkeletonShape, SkeletonText} from '@workday/canvas-kit-react/skeleton';
...
// v5
import {Skeleton} from '@workday/canvas-kit-react/text-input';
...
The skeleton animation has been updated from a diagonal sheen, or shimmer, to fading the opacity of
the entire shape(s) in and out.
