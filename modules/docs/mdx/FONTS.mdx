...
Canvas Kit doesn't include fonts by default. Which font package you need depends on whether your
application has the [Sana Canvas theme](#sana-sans) (`data-theme="sana-canvas"`) enabled.
...
The fonts package makes importing font files simple so you know you're using the correct font in
your environment. We use Sana Sans for most languages we support at Workday.
...
We currently host Sana Sans through durable storage, and this package connects `@font-faces` to
those font files. This keeps the package size minimal, while ensuring you don't have to worry about
the wiring. But because durable storage is regionally-bound, you might consider using a CDN if your
environment experiences degraded performance.
...
> **Note:** This requires `@workday/canvas-tokens-web` `4.4.0-beta.13` or later, where the base
> font-family token name was aligned with the `fonts` package's `@font-face` family name. If you're
> on an older version, upgrade `@workday/canvas-tokens-web` first.

Once the theme's font-family token and the loaded `@font-face` share the same family name, no
additional CSS override is required — install and use the `fonts` package exactly as described in
...
`@workday/canvas-kit-react-fonts` to load Roboto instead.
