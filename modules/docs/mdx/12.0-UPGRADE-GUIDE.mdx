...
- `FormField.LabelPosition.Hidden` is no longer valid. If you still want the label but not visible,
  you can use `accessibleHideStyles`
...
// v12 Newly promoted FormField from Preview to Main
import {accessibleHideStyles} from '@workday/canvas-kit-react/common';

//...

<FormField error="alert" orientation="horizontalStart" isRequired={true} id="input-id">
  <FormField.Label cs={accessibleHideStyles}>Search</FormField.Label>
  <FormField.Field>
    <FormField.Input as={TextInput} onChange={handleChange} value={value} />
  </FormField.Field>
</FormField>;
