...
We've deprecated `FormField.Container`. Please use `FormField.Field`.
...
🤖 The codemod will handle the change of updating the `size` prop and `variant` prop to the correct
type.
...
We've added a new sub component to `FormField`, `FormField.Field`. This sub component is meant to
ensure that the label is always aligned with the input regardless of the value of the `orientation`
prop on `FormField`. This component should replace `FormField.Container` and always be used to
ensure proper alignment. Our examples have been updated to reflect this.
...
Parts of Select have been enhanced to improve overriding styles. There should be no change to the
API or how the component is styled on the consumer end.
