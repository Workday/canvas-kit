...
We've deprecated `FormField.Container`. Please use `FormField.Field`, this ensures proper label
alignment regardless of the orientation.
...
`RadioGroup` or a group of `Checkbox`'s. `FormFieldGroup`.

`FormFieldGroup` supports the same props of `FormField`:

- `error`: `"alert" | "error"` Defines the error around the whole group of inputs.
- `orientation`: `"horizontalStart" | "horizontalEnd" | "vertical" `. Defines the label placement.
