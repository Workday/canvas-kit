...
We add the [@deprecated tag](https://jsdoc.app/tags-deprecated.html) from JSDoc to code that we plan
to deprecate in the near future. Although you can still consume this code, we want consumers to move
...
We are [deprecating](#deprecation) `Header` and `GlobalHeader` and their related exports. They have
been renamed to `DeprecatedHeader` and `DeprecatedGlobalHeader`, respectively. You may continue to
use these components exactly as you did in v5, but note that we plan to [remove](#removal) these
components in Canvas Kit v7.
