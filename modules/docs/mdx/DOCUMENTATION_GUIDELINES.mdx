...
Placeholder content throughout this document is enclosed in brackets as `[placeholder]`. For example, if
you were writing component documentation for `Tabs`, you would replace:
...
Every component MDX (with a few [exceptions](#component-families)) should follow the structure outlined below.
...
<!-- TODO: Figure out if we need to import the component -->
<!-- Import component -->
import {[Component]} from '@workday/canvas-kit-[preview-|labs-]react/[component]';
...
<!-- TODO: Figure out if we need the component prop for the Meta tag -->
...
> **Note:** Refer to [Tabs](/components/containers/tabs/) and [Modal](/components/popups/modal/) for good examples of how to write well-structured component documentation.

### Component Families

Occasionally, you may want to document multiple top-level components (i.e.,
_not_ subcomponents) on a single page. For example, we document `PrimaryButton`,
`SecondaryButton`, `TertiaryButton`, and `DeleteButton` on a single page for all
[Button](/components/buttons/button/) components since they're all similar.

Upon identifying a `ComponentFamily` (e.g., `Button`) to designate a label for the collection of
related components, you'll then follow the same document structure
defined above with slight modifications.

````md
import {Specifications, SymbolDoc} from '@workday/canvas-kit-docs';

<!-- TODO: Again, figure out if we need to import the components -->
<!-- Import components in component family -->
import {
  Component1,
  Component2,
  ...
} from '@workday/canvas-kit-[preview-|labs-]react/[componentfamily]';
...
<!--
Each section is named after the comopnent.
-->
### Component1
...
> **Note:** Refer to [Button](/components/buttons/button/) for a good example of how to write well-structured component family documentation.

## To-dos

- Update Pull Request template in .github folder
- Top portion of document will be stripped out on Canvas Site.
- Link to Google writing style guide.
- Consider fixing Pagination docs
