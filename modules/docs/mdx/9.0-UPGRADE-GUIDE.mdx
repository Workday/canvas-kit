...
[Side Panel](https://workday.github.io/canvas-kit/?path=/story/preview-side-panel--basic). Aware
that `SidePanel` from main package is planned to be deprecated and replaced by one from Preview. So
we suggest to use preview version of `SidePanel` until main version `SidePanel` hard deprecation.
