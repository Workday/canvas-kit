...
import depthTokenUpdate from './depth-tokens-update.png'
...
In `v7` we released an update to our `depth` tokens that were too bold and harsh for web applications. We've modified the `depth` token values
to be more subtle and improve visual design. These updates change the `box-shadow` value and not the `depth` value used pero component. These changes
affects all components that use a box shadow including: `Card, Toast, Dialog, Popup, Modal, Menu`.

<img src={depthTokenUpdate} atl="A before and after of depth token values" />
