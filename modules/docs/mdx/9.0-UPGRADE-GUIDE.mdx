...
We've promoted `Toast` from Labs (`@workday/canvas-kit-labs-react`) to the Main (`@workday/canvas-kit-react`) package.

- This component is a [compound component](/getting-started/for-developers/resources/compound-components/).
- It has a `mode` prop that allows for propert accessibility attributed to be defined.
- It's sub components allow for more flexibility and access to lower level elements.
