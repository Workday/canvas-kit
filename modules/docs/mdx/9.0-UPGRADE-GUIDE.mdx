...
We've hard deprecated the `Stack`, `HStack` and `VStack` components along with it's props
(`StackProps`, `HStackProps`, `VStackProps`, `StackStyleProps`). We recommend using the `Flex`
component and replace the `spacing` prop with the `gap`. You can also run
`npx @workday/canvas-kit-codemod softDeprecate/Stack`.
