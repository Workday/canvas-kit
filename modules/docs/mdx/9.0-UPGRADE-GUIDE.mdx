...
- [Component Deprecations](#component-deprecations)
  - [Drawer](#drawer)
  - [Layout and Column](#layout-and-column) - [Stack, HStack, VStack](#stack-hstack-vstack)
  - [Menu in Preview](#menu-in-preview)
...
We've removed the `Menu` component that was in `@workday/canvas-kit-preview-react` and now will be
replaced with the `Menu` component in `@workday/canvas-kit-react`. Please see the `Menu`
documentation
[here](https://workday.github.io/canvas-kit/?path=/docs/components-popups-menu--basic).

For the migration guide from `Menu` in `@workday/canvas-kit-preview-react` to the `Menu` component
in `@workday/canvas-kit-react`, see [this](https://github.com/Workday/canvas-kit/discussions/2063)
discussion.
...
> `Menu` being a compound component.
