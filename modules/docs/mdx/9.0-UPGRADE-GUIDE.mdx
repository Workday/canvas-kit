...
`useCanvasTheme` and `getCanvasTheme` have been removed and can be safely replaced by `useTheme` and
`getTheme`.
...
We've deprecated `useThemeRTL` from Labs. Although you may still use this utility, we recommend you
use
