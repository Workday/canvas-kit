...
use in product, but may not be up to the high code standards upheld in the Main package. Preview is
...
Generally speaking, our goal is to eventually promote code from Preview to Main. Occasionally, a
component with the same name will exist in both Main and Preview (for example, see Segmented Control
...
different API. The component in Main will eventually be replaced with the one in Preview.
...
### Stack, HStack and VStack - _Removal_
...
### useCanvasTheme and getCanvasTheme - _Removal_
...
### useThemeRTL - _@deprecated_

**PR:** [#2119](https://github.com/Workday/canvas-kit/pull/2119)

We've deprecated `useThemeRTL` from Labs. Although you may still use this utility, we recommend
using
...
🤖 The codemod will rewrite your usages of the previous `Toast` in Main to use the compound
...
We've promoted `useThemedRing` from Labs to Main. `useThemedRing` allows you to theme focus rings.
