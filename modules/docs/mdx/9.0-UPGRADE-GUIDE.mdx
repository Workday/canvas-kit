...
`useCanvasTheme` and `getTheme` have been removed as it's can be safely replaced by `useTheme` and
`getTheme` and there is no external usage of this functions found.
