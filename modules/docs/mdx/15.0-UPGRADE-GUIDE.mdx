...
- [Component Promotions](#component-promotions)
  ...
  - [Side Panel](#side-panel)
    - [Migrating from Preview](#migrating-from-preview)
    - [Hook API Changes](#hook-api-changes)
...
- [Component Updates](#component-updates)
  - [Buttons](#buttons)
    - [Tertiary Button v14 Style Toggle](#tertiary-button-v14-style-toggle)
...
- [New Utilities](#new-utilities)
  - [colorSpace](#colorspace)
      - [Darken](#darken)
      - [Hover and Pressed](#hover-and-pressed)
- [Glossary](#glossary)
  - [Main](#main)
  - [Preview](#preview)
...
#### Tertiary Button v14 Style Toggle

**PR:** [#3875](https://github.com/Workday/canvas-kit/pull/3875)

For Canvas Kit v15, `TertiaryButton` supports an optional data attribute, `data-dsr-off`, that
reverts the component to v14 styles. This allows consumers to more gradually transition
between major versions for this component if needed. By default, this toggle is set to `undefined`. It must be
explicitly set to `true` to revert styles. Support for `data-dsr-off` is **temporary** and will become inert
...
- `SvgProps` no longer extends `BoxProps`, which means Icons components may not accept Box-related
  style props. Switch to use `cs` prop instead, use `v14.1 codemod` to migrate style props
  automatically.
- The stencil variables for `width` and `height` are now deprecated; use size prop instead.
...
- Added a new `inverse` prop, enabling an inverse color variant for better adaptability across
  backgrounds.
- The `background` prop and a `color` prop must be provided together to ensure optimal contrast and
...
The new **Expressive Icon** component brings expressive icons to Canvas Kit. This component replaces
...
> This can be done through a `v15-icons codemod` for auto-replacement or follow
...
[Switch](https://workday.github.io/canvas-kit/?path=/docs/preview-inputs-switch--docs) component in
`Preview`.
...
For more information on theming, view our
[Theming](https://workday.github.io/canvas-kit/?path=/docs/features-theming-overview--docs)
documentation.
...
We've removed the `ai` variant from the `StatusIndicator` component in `Preview`. This pattern is no longer supported. If you wish to create your own custom variant, you can do so by using the `variant` prop and passing in a custom color. Please refer to the custom variant documentation [here](https://workday.github.io/canvas-kit/?path=/docs/preview-status-indicator--docs#custom) for more information.
...
We have added a new utility called [colorSpace](https://workday.github.io/canvas-kit/?path=/docs/styling-utilities--docs#colorspace) and it has three functions: [darken](https://workday.github.io/canvas-kit/?path=/docs/styling-utilities--docs#darken), [hover](https://workday.github.io/canvas-kit/?path=/docs/styling-utilities--docs#hover) and [pressed](https://workday.github.io/canvas-kit/?path=/docs/styling-utilities--docs#pressed).

These utilities are meant to be used for interactive states (i.e. `:hover`, `:active` on buttons, links, etc.).

These will return [color-mix()](https://developer.mozilla.org/en-US/docs/Web/CSS/Reference/Values/color_value/color-mix) and the result is mixing the first color and the mixin color together in the srgb colorspace by a given amount.

Why did we create this utility?
...
1. `color`: The color that will be darkened (this is typically the "base" color on the given
   element).
2. `fallback`: A fallback color if the first color is not valid or not defined.
3. `colorType`: A string that will determine where the mixin color and the mixin percentage comes
