...
If you theme via **CSS** (e.g. with `createStyles` and a `className` on `CanvasProvider`), that automatic mapping does not run. You must:

1. **Set the brand token** to your desired color value.
2. **Set the system brand token** to reference the brand token (so components that use system tokens still pick up your theme).
