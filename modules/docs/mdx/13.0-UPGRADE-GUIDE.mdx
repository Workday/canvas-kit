...
- `maxWidth` has been removed from the `usePillModel`. This config was used to style sub components. With the refactor to use `data-part` and stencils, it is no longer needed on the model. You can still apply `maxWidth` on the parent `<Pill>` element and the child elements should style accordingly.
...
- `Pill.IconButton` now requires `aria-label` as a prop. The default `aria-label="remove"` was removed to ensure consumers pass a valid value.
