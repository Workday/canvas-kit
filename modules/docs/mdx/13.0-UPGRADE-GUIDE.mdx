...
- We've removed extra elements and leverage flex box to ensure only the label receives overflow styles. When maxWidth is set on the Pill the maxWidth is set for the whole Pill accurately. Before v13, the maxWidth wasn't calculating the width of all its elements and wasn't a true pixel value.
