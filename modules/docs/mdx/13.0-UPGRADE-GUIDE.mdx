...
We've updated the `jsx` flag in our `tsconfig` to `react-jsx` to provide better support for newer technologies like `Vite`. As part of this change, we're also updated our peer dependencies of `React` to a minimum version of `17.0.0`.

### Refactored Components to Use our Styling API

We've continued to refactor our component to use our new Styling API and CSS Tokens. This allows us to provide a more consistent styling experience across our components.

### Provide New Branding Logos

As part of a brand refresh, we now provided updated logos that reflect the new Workday branding.
...
**After in v13**
```tsx
// You must provide an altText that makes sense for the context in which the Avatar is used
<Avatar altText="My Avatar" as="div" />
```
...
- The `iconLabel` prop no longer has a default of `"Opens link in new window"`. The default was an English phrase (Opens link in new window) and this has caused issues in the past with translations.
...
- We've removed extra elements and leverage [flex box](https://developer.mozilla.org/en-US/docs/Learn_web_development/Core/CSS_layout/Flexbox) to ensure only the label receives overflow styles.
- When `maxWidth` is set, it is set on the parent `<Pill/>` element and the child elements will be styled accordingly. Before v13, `maxWidth` wasn't calculating the width of all its elements and wasn't a true pixel value.
...
- Our components rely on the `@workday/canvas-tokens-web` package to ensure correct styling and use of CSS variables. Ensure you're upgrading to the latest version and installing it correctly. For more information, view our [docs](https://workday.github.io/canvas-tokens/?path=/docs/docs-getting-started--docs).
