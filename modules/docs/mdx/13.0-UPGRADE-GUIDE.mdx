...
any questions. For common upgrade issues, please see the Troubleshooting section at the end of this doc.

## Why You Should Upgrade

### Improved Support for ESModules

We've updated the `jsx` flag in our `tsconfig` to `react-jsx` to provide better support for newer technologies including `Vite`. As part of this change, we've also updated our `react` peer dependencies to a minimum version of `17.0.0`.

### Refactored Components to Use Our Styling API

We've continued refactoring our components to use our new Styling API and CSS Tokens. This allows us to provide a more consistent styling experience across our components.

### Provide New Brand Logos
...
Having trouble running our codemods? View our [docs](https://workday.github.io/canvas-kit/?path=/docs/guides-codemods) for more information!
...
- The `iconLabel` prop no longer has a default of `"Opens link in new window"`. This default phrase caused some to accidentally omit translations which caused translation issues.

>**Note:** You *must* provide an `iconLabel` for `<ExternalHyperlink />` to ensure proper accessibility.  Our examples have been updated to reflect this change.
...
- We've removed extra elements and now use [flex box](https://developer.mozilla.org/en-US/docs/Learn_web_development/Core/CSS_layout/Flexbox) to ensure only the label receives text overflow styles when needed.
- Setting `maxWidth` on the parent `<Pill/>` element now styles the child elements correctly. Before v13, `maxWidth` wasn't calculating the width of all its elements and wasn't a true pixel value.
...
- `<Pill.Label/>` is a *required* element when using other subcomponents such as `<Pill.Icon/>` to ensure that the label truncates correctly.
...
> 🤖 The codemod will handle the change adding `<Pill.Label/>` if `<Pill.Icon/>` or `<Pill.IconButton/>` is being used.
...
> **Note:** You'll need to update the way `jsx` transpiles. The automatic runtime feature, enabled through the `@babel/preset-react` preset with the `runtime: "automatic"` option. This handles the importing of functions that JSX transpiles to. For more information please [view Babel's docs](https://babeljs.io/docs/babel-preset-react#react-automatic-runtime) on automatic runtime for React.


## Troubleshooting

- When upgrading to the latest major version of Canvas Kit, all related Canvas Kit packages should be updated at the same time:
	...
	- `"@workday/canvas-kit-preview-react": "^12.5.6"` -> `"@workday/canvas-kit-preview-react": "^13.0.0"`
	- `"@workday/canvas-kit-labs-react": "^12.5.6"` -> `"@workday/canvas-kit-labs-react": "^13.0.0"`
- Our components rely on the `@workday/canvas-tokens-web` package which provides our CSS variables and ensures the correct styling of our components. Make sure to upgrade to the latest version of Canvas Tokens Web and install it correctly. For more information, [view our docs](https://workday.github.io/canvas-tokens/?path=/docs/docs-getting-started--docs).
