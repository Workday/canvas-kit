...
We will be upgrading to Emotion 11 and React 17 as part of the v7 release. This will allow teams to
upgrade to newer versions of Emotion and React without the need to maintain older versions in order
to use Canvas Kit.

As part of this upgrade, Canvas Kit will remove the jsx pragma and the use of CSS props. However,
consumers can continue to use CSS props.

To use v7, teams will need to be using:

- React 16.14 OR React 17.X for backwards compatibility on JSX transform if they are using Babel or
...
Please note that teams may also need to update any side effect dependencies or linting packages with
this update.

View our [React 17](https://github.com/Workday/canvas-kit/discussions/1455) and
[Emotion 11](https://github.com/Workday/canvas-kit/discussions/1453) Github discussions to learn
more about any gotchas or tips and tricks with these upgrades. And of course, feel free to
contribute to the discussion with any questions or learnings of your own!
...
Note that the APIs for these promoted components remain unchanged.
...
Like all components that use the
...
module, `Banner` now supports ref forwarding and using the `as` prop to change the component's
underlying element.

The `variant` prop (which previously accepted `Banner.Variant.Full` or `Banner.Variant.Sticky`) has
been converted to the boolean `isSticky` prop on the container `Banner` component. Similarly, the
`error` prop (which previously accepted `Banner.ErrorType.Alert` or `Banner.ErrorType.Error`) has
been converted to the boolean `hasError` prop on `Banner`.
...
Each button now supports the following sizes: `extraSmall`, `small`, `medium` and `large` (defaults
to `medium`).
