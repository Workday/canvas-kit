...
our `PrimaryButton`, `SecondaryButton` and `TertiaryButton`. \_

Note\_: In order to render a icon button, omit any `children` and pass the button an `icon` prop.
...
This will render an icon button that is styled like our secondary buttons.

### Button Mapping

Below is a mapping of `IconButtons` to their respective buttons. _Note_: We're no longer supporting
what use to be `plain`, `square` and `squareFilled`. If you would still like to have a squared icon,
you can use
...
You can now render what use to be an `IconButton` using out main buttons.
...
Each button now supports sizes: `extraSmall`, `small`, `medium` and `large`
