...
disclosure-type models has been updated to remove the extra event wrapper. This change allows
developers to directly attach the `show` and `hide` events directly to React event handlers.
...
the wild, so impact will be minimum. Most usage of `show` or `hide` events are called without
`event`, so this change will not impact most people. There is no codemod for this change, because
the usage is much more difficult to detect since most people pass a callback that doesn't take
parameters.
