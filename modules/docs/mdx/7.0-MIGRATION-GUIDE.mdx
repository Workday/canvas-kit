...
to automatically update your code to work with a majority of the breaking changes as you migrate
...
- Preview Side Panel (alternate variant): Depth 3 → Depth 5
...
You'll need to update your imports for the promoted components (this is _not_ handled by the
codemod):
...
To consolidate Button APIs, we've removed `IconButton` in favor of `PrimaryButton`,
`SecondaryButton` and `TertiaryButton`.
...
The following changes will not be automatically updated and will require a manual update:
...
below is a description of how to manually migrate from unsupported v6 variants.

| IconButton variant | Button Type                          |
| ------------------ | ------------------------------------ |
| `circle`           | TertiaryButton                       |
| `circleFilled`     | SecondaryButton                      |
| `inverse`          | TertiaryButton with variant inverse  |
