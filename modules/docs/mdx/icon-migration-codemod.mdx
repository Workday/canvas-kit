...
## Manual migration and lookup

If you need to look up a mapping without running the codemod, open
`node_modules/@workday/canvas-system-icons-web/dist/metadata/system.deprecated.metadata.json`. Each
entry includes the following fields:

- The `name` field is the kebab-case icon name. It maps to `nameIcon` in camelCase exports.
- The `fallback` field is the filename of the replacement icon. For example,
  `wd-icon-arrow-up-to-line.svg` maps to `arrowUpToLineIcon`.

You can also browse system icons and migration guidance on
[Canvas Design](https://canvas.workday.com/styles/assets/system-icons).
