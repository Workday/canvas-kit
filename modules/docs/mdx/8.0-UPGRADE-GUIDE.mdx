...
We've added a set of [responsive utilities](/docs/features-responsive-styling--responsive-container)
to facilitate container-based and viewport-based responsive styling:
...
which has been redesigned as a [compound component](/story/guides-compound-components--page) to the
Preview package. `SegmentedControl` represents a linear group of multiple buttons allowing the
selection of a specific value and is best used for switching between different views of the same
content.
...
which has been redesigned as a [compound component](/docs/guides-compound-components--page) to the
Preview package. `StatusIndicator` is best used for helping the user quickly identify the status of
a task, action, or page element.
...
[Collection API](/docs/features-collections--basic) which provides new overflow behavior based on
container width.
