...
This change will not impact product teams that are already on v7 and are looking to upgrade to v8 as
v8 will support the same React versions as v7 while still giving teams the ability to upgrade to
React 18 as needed. Canvas Kit will continue to support React 16.14, React 17.X in v8, so it allows
to upgrade to Canvas Kit v8 without upgrading to React 18. But some new features may be ignored and
limited by the legacy root API usage and a console warning about `.root()` being deprecated will be
presented.

You can read more about this upgrade in the
[discussion](https://github.com/Workday/canvas-kit/discussions/1545)
