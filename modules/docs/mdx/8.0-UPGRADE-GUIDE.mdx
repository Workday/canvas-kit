...
**Impact on Canvas Kit:** Updating to React 18 didn't cause any breaking changes in our code. However, we did have to update some types based on some type packages that where upgraded.
To see React v18 features in action we had to upgrade Storybook to v6.5.x.
