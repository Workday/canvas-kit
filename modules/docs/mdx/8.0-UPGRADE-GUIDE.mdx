...
If you're unable to run the codemod successfully using `npx`, try installing the codemod package as
a dev dependency, running it using `yarn` instead of `npx`, and then removing the package after
you're finished.
...
Beginning with v8, Canvas Kit no longer supports IE11.

We've removed all IE11-specific code including polyfills. Applications built using Canvas Kit v8 and
above may no longer run in IE11 and fail to bootstrap with an error and/or a white screen.
...
> (PLACEHOLDER) Soft deprecation of Drawer, Menu (Preview), Layout and Column goes here.
> (https://github.com/Workday/canvas-kit/pull/1747)
