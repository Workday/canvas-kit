...
overflow behavior provided by the Collection API. In order to enable the new overflow behavior, you'll
need to use the dynamic API by passing an array of items to `Breadcrumbs` (rather than passing the
items as statically defined `children` via JSX). The items can be passed to `Breadcrumbs` via its
`items` prop or via an `items` key within a provided `model`.
