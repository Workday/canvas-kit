...
`Breadcrumbs` has been promoted to the Main (`@workday/canvas-kit-react`) package. It now uses the
list system from our [Collection API](/getting-started/for-developers/resources/collection-api/)
which provides new overflow behavior based on container width.

The following changes have been made to the `Breadcrumbs` API:
...
There are a couple additional updates to the `Breadcrumbs` API which will require manual changes.

- Handling Redirects
- Updated Overflow Behavior

We've outlined them in more detail below.
