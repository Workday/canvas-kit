...
_Note_: This change is using `breakpoints` defined by a default canvas theme.

There arew a couple ways to make sure this change works in your application.

- You can wrap a piece of code or your application in a
  [CanvasProvider](https://github.com/Workday/canvas-kit/blob/master/modules/react/common/README.md#canvasprovider),
  which is a higher order component intended for theming, RTL support and a `InputProvider`.
...
If you want to customize any part of the theme there are two way to do it.

You can spread `defaultCanvasTheme` and then adjust your theme. This will ensure any missing
properties are merged in:
...
<CanvasProvider
		theme={{
			canvas: {
				...defaultCanvasTheme,
				direction: state.isRTL ? ContentDirection.RTL : ContentDirection.LTR,
			},
		}}>
...
The other way is by using `useTheme` which merges any missing properties from the `theme` object.
...
const MyComponent = () => {
	...
	return (
		<CanvasProvider theme={theme}>{/* All your components containing any Canvas components */}</CanvasProvider>;
	)
}
```
