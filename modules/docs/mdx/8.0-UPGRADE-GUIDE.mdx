...
We are [soft deprecating](#soft-deprecation) `SegmentedControl` and it has been renamed to
`DeprecatedSegmentedControl`. Similarly, `SegmentedProps` has been renamed to
`DeprecatedSegmentedProps`. You may continue to use this component, but note that we plan to
[hard-deprecate](#hard-deprecation) it in Canvas Kit v9.

🤖 The codemod will rename `SegmentedControl` and `SegmentedControlProps` to their deprecated names:
