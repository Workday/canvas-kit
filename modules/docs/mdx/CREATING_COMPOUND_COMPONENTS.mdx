...
We can see that the `DisclosureProps` interface extends the `DisclosureConfig` interface. This allows us to pass model config directly to the `<Disclosure>` component. A user of this `<Disclosure>` component might want to register a callback when the `open` event is called, for instance.
...
The `DisclosureTarget` component is in charge of the toggle button and it calls the `open` or `close` event on the model.
