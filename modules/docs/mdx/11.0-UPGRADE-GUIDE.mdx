...
We've promoted the `Table` component in preview to main.
...
We've removed the `Table` component in main and replaced it with the `Table` component from preview.
This will consist of a new API since it is now a compound component.
...
In v10, `Table` and `TableRow` were the pieces that would be used to build a Table.
In v11, `Table` is now a compound component that utilizes all the pieces of an html
[Table](https://developer.mozilla.org/en-US/docs/Web/HTML/Element/table) element to build a Table.
...
> **NOTE: This will have to be updated manually as the API is too different for a codemod.**
