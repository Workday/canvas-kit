...
Components now support the `cs` prop, instead of removed `styles` prop.

**Note: Passing token name to a prop will be not supported in a future version. Instead of
`color="blueberry100"` consider using actual token `color={base.blueberry100}`**
...
Only impact Canvas Kit. `Icon` component has been removed and no longer used in icons. It was
replaced by `Svg` and icon stencils extend `svgStencil`.
