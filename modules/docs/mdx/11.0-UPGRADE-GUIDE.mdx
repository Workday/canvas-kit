...
We've promoted the `Table` compound component in [Preview](#preview) to [Main](#main).
...
We've removed the `Table` component in [Main](#main) and replaced it with the `Table` component from [Preview](#preview).
This will consist of a new compound component API.

> `rowState` prop is no longer a part of the `Table` component.

In v10, the main `Table` only exposed a `Table`  and `TableRow` component to build tables.
...
> **NOTE:** If you were using the `preview` table, then you will need to update your import from `"@workday/canvas-kit-preview-react/table"` to `"@workday/canvas-kit-react/table"`.
