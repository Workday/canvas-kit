...
**PR:** [#2472](https://github.com/Workday/canvas-kit/pull/2620)

#### statusIndicatorColors

We've deprecated `statusIndicatorColors` for StatusIndicator because it's now directly handeled by
the `statusIndicatorStencil`.

#### useStatusIndicator

We've deprecated `useStatusIndicator` for StatusIndicator because it's now directly handeled by the
`statusIndicatorStencil`.

#### useStatusIndicator

We've deprecated `useStatusIndicatorIcon` for StatusIndicatorIcon because it's now directly handeled
by the `statusIndicatorStencil`.
