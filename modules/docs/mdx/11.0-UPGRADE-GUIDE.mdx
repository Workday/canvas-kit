...
`variant`, `disabled` and styles props. `typeLevel` prop has been removed from `LabelText` and we
suggest to use type level components, `Title`, `Heading`, `BodyText` and `Subtext`, instead.

🤖 The codemod will handle a `typeLevel` removal for you automatically.
