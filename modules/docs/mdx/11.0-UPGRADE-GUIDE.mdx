...
Opacity which was applied to the whole container has been removed for the `transparent` variant and
replaced by a translucent color. Opacity has changed from `0.85` to `0.84`. There is possibility of
a small visual change and it should be accepted in visual regression tests.
