...
The `variant` prop on `SidePanel` no longer accepts `alternate`. It has been renamed to `modal` to
...
// After
<SidePanel variant="modal">{/* ... */}</SidePanel>
```

> 🤖 The `v16` codemod will rename `variant="alternate"` to `variant="modal"` as shown above. It
...
switch to the new `alternative` variant instead of `modal` to avoid picking up the overlay shadow.
...
| `modal`       | `system.legacy.color.surface.default`    | 6     |

#### Visual Updates

- The `modal` variant's box-shadow updated from `system.depth[3]` to `system.depth[6]`, matching
  `Modal.Card` (see [Modal](#modal)).
