...
`sanaCanvasProviderTheme` to CanvasProvider. Popups naturally inherit CSS variables from the
global theme.
...
// Simpler setup - no theme prop needed
<CanvasProvider>
  <App />
</CanvasProvider>
```

The `sanaCanvasProviderTheme` export remains available for edge cases like testing environments
without global CSS or custom popup containers, but is no longer required for typical usage.

If you see a console warning about unnecessary theme usage, you can safely remove the theme prop.
