...
We've added a `neutral` variant: underlined dark-gray link text (`fg.default` → `fg.strong` on
hover/active). Use it as an alternate to the default blue link — for example on colored backgrounds.
Unlike `standalone`, it keeps the underline.
...
> **Note:** Prefer `standalone` / `standaloneInverse` when the link sits outside body text and
> should not be underlined (e.g. breadcrumbs). Prefer `neutral` when you want an underlined
