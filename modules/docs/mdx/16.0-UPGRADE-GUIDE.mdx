...
- Default link color uses Sana brand link tokens (`system.sana.color.brand.fg.link.default` /
  `.hover`). Hover and active text is now the darker link-hover blue.
- Hover and active backgrounds use surface overlay tokens instead of `surface.alt`.

##### New Secondary Variant and `type` Prop

A new "secondary" variant and `type` prop have been added to `Hyperlink` and `ExternalHyperlink`.
Color and underline are now controlled by separate props:

| Need                       | API                                       |
| -------------------------- | ----------------------------------------- |
| Blue, underlined (default) | _(no props)_                              |
| Blue, no underline         | `type="standalone"`                       |
| Inverse, underlined        | `variant="inverse"`                       |
| Inverse, no underline      | `variant="inverse"` `type="standalone"`   |
| Dark gray, underlined      | `variant="secondary"`                     |
| Dark gray, no underline    | `variant="secondary"` `type="standalone"` |

`variant` is now either 'inverse' or 'secondary'. Underline is controlled by `type` which is either
'inline' or 'standalone', defaulting to 'inline'. The compound variants `standalone`, and
...
| Before                        | After                                   |
| ----------------------------- | --------------------------------------- |
| `variant="standalone"`        | `type="standalone"`                     |
| `variant="standaloneInverse"` | `variant="inverse"` `type="standalone"` |

> **Note:** The composed `variant="inverse"` `type="standalone"` focus ring uses the inverse focus
> style from the `inverse` variant. The former `standaloneInverse` variant used
> `system.legacy.color.focus.inverse` — expect a small focus-ring difference after migration.
