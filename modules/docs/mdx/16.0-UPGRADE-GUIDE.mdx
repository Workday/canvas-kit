...
elements are not enough for portaled popups (menus, selects, modals) — those render under
`document.body` and only inherit theme from `<html>`, or from `sanaCanvasProviderTheme` on
`CanvasProvider` when `<html>` is unavailable.
