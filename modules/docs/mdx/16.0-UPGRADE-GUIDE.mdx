...
`sanaCanvasProviderTheme` to CanvasProvider. Popups inherit CSS variables from the document.
...
**Still required — no access to `<html>`:** embedded apps, microfrontends, and third-party shells
often cannot set attributes on `<html>`. Nested `data-theme` does not reach portaled popups. Keep
passing `sanaCanvasProviderTheme` so menus, selects, and modals get Sana brand variables:
...
instead. While the styles are different, both were intended to be used on an `alt` background.
