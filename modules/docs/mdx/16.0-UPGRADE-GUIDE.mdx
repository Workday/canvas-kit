...
- Text color updated from `system.color.fg.default` to `system.color.fg.strong`.
- Built-in icons (`Menu.Item.Icon`, `Menu.Option` checkmark, `Submenu.TargetItem` chevron) now use
  `size="xs"` and updated icon assets (`checkIcon`, `chevronRightIcon`).
...
| Focus                           | Solid blue fill (`brand.accent.primary`)                                              | Inset outline using `brand.border.primary`                                             |
| Selected                        | Solid blue fill and text (`brand.surface.primary.strong` / `brand.fg.primary.strong`) | Light blue background and blue text (`brand.surface.selected` / `brand.fg.selected`)   |
| Selected + Focus                | Solid blue fill                                                                       | Inset outline with selected background preserved                                       |
| Disabled + Focus                | Solid blue fill (`brand.accent.primary`) with disabled opacity (dims focus style)     | Light blue background and blue text with disabled opacity (dims selected style)      |
| Expanded submenu parent + Focus | Same blue fill focus style as other focused items                                     | Focus ring removed; selected styling shown instead to indicate the open ancestor trail |
