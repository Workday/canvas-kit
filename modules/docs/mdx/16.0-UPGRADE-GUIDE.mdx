...
- `Expandable.Content` padding is now uniform `padding.xs` (8px) on all sides (previously `padding.md`
  top / `padding.xs` sides and bottom).

No API changes. No migration is required unless you override `expandableTargetStencil` or
`expandableContentStencil` styles directly.
...
- Column header label text color is now `system.color.fg.strong` (previously inherited
  `fg.default` from the table container).

No API changes. No migration is required unless you override `baseTableStencil` or
`baseTableHeaderStencil` styles directly.
...
`Card`, `Expandable` (`Expandable.Target`), `Menu` (`Menu.Card`, `Menu.List`, `Menu.Item`), and
`MultiSelect` extend this stencil for their rounded corners.
