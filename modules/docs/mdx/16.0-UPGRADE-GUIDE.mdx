...
- `Link` now uses the newly introduced `secondary` Hyperlink variant (see
  [Hyperlink](#hyperlink)).
...
- Label typography now uses the `subtext.medium` type level (12px, previously `subtext.large`, 14px)
  with `fontWeight.medium` (previously `fontWeight.bold`), and no longer applies
  `text-transform: capitalize`.
- The `transparent` variant's label now has a text-shadow applied for legibility over imagery or
  video backgrounds.
