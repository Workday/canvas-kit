...
- Background color tier changed from `surface.alt.strong` to `surface.alt.default` (a lighter
  ...
  states are unchanged from v15 and still mix from `surface.alt.strong`, since `color-mix()` doesn't
  produce a visible change when mixed against the new translucent `alt.default` base.
