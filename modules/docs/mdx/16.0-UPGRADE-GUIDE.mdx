...
- `ctaPlacement="bottom" | "end"` — `bottom` (default) stacks the link below heading/body; `end`
  places the link in a right column, vertically centered beside the content.
...
- Vertical spacing between text blocks: **4px** between heading and body (`gap.xs`), **16px** between
  body and link (`gap.md`).
- The default `InformationHighlight.Icon` size has been reduced from `24px` to `20px` (`systemIcon`
  size `md`).
- `Heading` typography now uses `system.legacy.type.subtext.lg` with `fontWeight.medium`
  (previously `body.sm` scale with `fontWeight.bold`). Color changed from `fg.default` to
  `fg.strong`.
