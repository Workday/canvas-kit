...
often cannot set attributes on `<html>`. Nested `data-theme` does not reach portaled popups
(including all Canvas Kit popups such as menus, selects, modals, and toasts). Keep passing
`sanaCanvasProviderTheme` so those surfaces get Sana brand variables:
