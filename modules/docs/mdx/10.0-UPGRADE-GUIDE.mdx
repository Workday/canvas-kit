...
[`Table`](https://workday.github.io/canvas-kit/?path=/docs/preview-table--basic) in Preview instead.
...
We've refactored how we style buttons to use our `createStyles` utility function. We don't
anticipate this being a breaking change in most cases, but there may be slight changes to visual
tests.
...
We found that focus rings were not consistent across all buttons. We've updated our buttons to
display consistent focus rings across all variants. These will need to be taken note of due to some
small changes visually with Buttons.
