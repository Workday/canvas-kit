...
We've deprecated `Select` from [Preview](#preview). Please consider using [`Select`](https://workday.github.io/canvas-kit/?path=/story/components-inputs-select--basic) in [Main](#main) instead.
...
All Popup components including `Menu` and `Popup` have increased the top and bottom spacing between the target and popup to `4px`.
...
This component also does not havea border around the `Menu` component as the [Select in Preview](https://workday.github.io/canvas-kit/?path=/story/preview-select-left-label--default) has.
