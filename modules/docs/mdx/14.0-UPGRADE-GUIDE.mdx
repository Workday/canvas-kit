...
The `CountBadge` component has been updated in v14 with new styling options and improved
flexibility. The default `variant` and `inverse` styles have been updated to match new branding.
Additionally, a new `emphasis` prop has been introduced, allowing you to choose between `high` and
`low` emphasis for the badge.

By default, `CountBadge` uses the `default` variant and `high` emphasis, which is suitable for most
use cases. The `inverse` variant should be used when placing the badge on accent or dark backgrounds
to ensure proper contrast. When using the `inverse` variant with `low` emphasis, use caution: this
combination is only recommended on backgrounds with
[a token of 400 or greater](https://workday.github.io/canvas-tokens/?path=/docs/docs-base-tokens--docs)
to maintain sufficient contrast and readability.

**Count Badge variants in v14**

```tsx
<CountBadge /> // Default count badge
