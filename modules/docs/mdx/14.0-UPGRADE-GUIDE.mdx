...
color system. Although this dependency upgrade is not required, we've offered migration guides and codemods separately from the v14 upgrade so that teams can upgrade when they can.

> **Note:** v3 Tokens should be backwards compatible with older versions of Canvas Kit.

Follow these guides to migrate:

- If you are still using our old tokens from (`@workday/canvas-kit-react/tokens`): Migrate from `@workday/canvas-kit-react/tokens` to `@workday/canvas-tokens-web` v3:
