...
**PR:** [#3407](https://github.com/Workday/canvas-kit/pull/3407)

#### Breaking Changes

Canvas Provider has been updated to **remove** default branding colors to ensure proper cascading of our CSS variables. If you are using the `theme` prop to provide a custom theme, you will need to provide **all** properties of the palette.

** Before in v13**
In v13, the `useTheme` hook would call `createCanvasTheme` which generated a palette given a `main` color.
...
The reason for this change is to ensure that the CSS variables are properly cascaded to all components. Before in v13, the `CanvasProvider` would add the brand tokens under a class name, creating a higher specificity.

- `CanvasProvider` has been updated to include a `lighter` color.
