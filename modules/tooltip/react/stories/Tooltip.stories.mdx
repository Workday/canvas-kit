...
Tooltips can be applied to text that truncates with an ellipsis. To achieve ellipsis text, you must
have the following CSS applied to an element:
...
An `OverflowTooltip` can be applied to any element that has this overflow applied, or has a child
element that has this overflow applied.
