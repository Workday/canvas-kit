...
[Compound Components](../../../../COMPOUND_COMPONENTS.md), which means it can be composed in a
...
`onActivateTab` callback. This example is the same, but with named keys for the tabs and panels.
...
```tsx
// configuration
<Tabs onActivateTab={({ data }) => console.log('Activate tab', data.tab)}>
