...
export declare namespace StateMachine {
  ...
  export type GuardMap<TContext extends object, TEvent extends EventObject> = Record<
    string,
    Exclude<Guard<TContext, TEvent>, string>
  >;

  export type GuardFunction<TContext extends object, TEvent extends EventObject> = (
    context: TContext,
    event: TEvent
  ) => boolean;

  export type Transition<K extends string, TContext extends object, TEvent extends EventObject> =
    | K
    | {
        target?: K;
        actions?: SingleOrArray<Action<TContext, TEvent>>;
        cond?: SingleOrArray<Guard<TContext, TEvent>>;
      };
      ...
      [key in TState['value']]: {
        on?: {
          [K in TEvent['type']]?: SingleOrArray<
            Transition<TState['value'], TContext, TEvent extends {type: K} ? TEvent : never>
          >;
        };
  ...
  export type StateListener<T extends State<any, any, any>> = (state: T) => void;
