...
describe('styleParser', () => {
  ...
  describe('createStencil', () => {
    it('should convert an object `base` to a pre-computed style', () => {
      const program = createProgramFromSource(`
    ...
    it('should convert an object `base` to a pre-computed style if returned from an ArrowFunction with a ParenthesizedExpression', () => {
      const program = createProgramFromSource(`
        import {createStencil} from '@workday/canvas-kit-styling';
      ...
      expect(result).toContain('styles: "color:var(--button-color);"');
    });
  });
});
