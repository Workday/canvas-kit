...
import {AlternativePanel, ModalPanel} from './examples/Variant';
...
| `variant="alternate"`                          | `variant="modal"`                                    |
...
| `modal`       | `system.legacy.color.surface.default`    | 6     | Panels that float above page content, such as a panel with overlay behavior.  |
...
The `modal` variant uses the default surface color with depth 6, matching `Modal.Card`. Use it when
the panel overlays page content instead of sitting alongside it. Note that the depth is the only
overlay affordance `SidePanel` provides — you're responsible for the positioning, backdrop, and
focus management that overlay behavior requires.

<ExampleCodeBlock code={ModalPanel} />
