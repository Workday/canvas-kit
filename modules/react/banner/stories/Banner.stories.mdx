...
Use the children of `Banner.Action` to customize the action text contained in the `Banner`. The text
has default value of `View All`.
...
the `Banner.Action` will be hidden. Some basic styles will be applied, but you will still need to
manually set the css `position` value.
...
`Banner.Action` is a styled `<Box>`. This component will get an id that will be used for the
...
```tsx
<Banner.Action>Custom call to action</Banner.Action>
```
