...
`mode`, you need to add an `aria-label`. This `aria-label` should be like a category label for the
`Toast`. The `Toast` will also add an `aria-describedby` that links the `Toast` to `Toast.Message`
so that it is read out to screen readers.

<ExampleCodeBlock code={ToastDialog} />

The `Toast` will be given a role of `dialog`. You need to provide an `aria-label` to indicate the
