...
<ExampleCodeBlock of={ToastStories.Basic} />
...
<ExampleCodeBlock of={ToastStories.WithPopper} />

### Alert

Set the `mode` prop to `alert` to convey urgent and important information such as an error.

<ExampleCodeBlock of={ToastStories.ToastAlert} />
...
<ExampleCodeBlock of={ToastStories.ToastDialog} />
...
<ExampleCodeBlock of={ToastStories.WithActionLinkAndCloseIcon} />

### Right-to-Left (RTL)

`Toast` supports right-to-left languages when specified in the `CanvasProvider` `theme`.

<ExampleCodeBlock of={ToastStories.RTL} />
