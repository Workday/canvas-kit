...
with little additional configuration. As seen in the example below, you don't need to pass any accessibility attributes, because they are
baked into the component. You also can specifc `aria-label` attribute to a `nav` component (default value is `breadcrumbs`) through a `aria-label` prop of `Breadcrumbs` and `aria-label` to overflow button through `overflowButtonProps` of `Breadcrumbs.List` components.
...
`Breadcrumbs.List` is a `Flex` component rendered as a `<ul>` element. It is a container for `Breadcrumbs.Item` subcomponents.
...
might want to use the internal SPA router instead. You can override this hard-redirect with the
`onAction` prop. Note that `onAction` will not block any `onClick` if provided. The example below
uses the `history` package.
