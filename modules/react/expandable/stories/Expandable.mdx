...
Example](#basic-example): **`Expandable.Target`**, **`Expandable.Icon iconPosition="start"`**,
**`Expandable.Title`**, **`Expandable.Content`** as a `div`). **Ask which `headingLevel` matches the
page outline** when the expandable titles body content; omit **`headingLevel`** for navigation. If
the spec is several related headers, use the **accordion** composition, not several copies of the
Basic Example with default `div` content. Omit **`Expandable.Avatar`**, **`as="section"`** /
`aria-labelledby` (unless accordion), a hoisted model (unless accordion or expand-all),
**`initialVisibility`**, and a custom **`id`**.
...
- **Accordion** — several related headers that can be open at once. [Hoisted Model](#hoisted-model)
  and the [Accordion Pattern](https://www.w3.org/WAI/ARIA/apg/patterns/accordion/). Each Content is
  **`as="section"`** with `aria-labelledby`.
- **Navigation list** — disclosed links in a list. [Side Panel with
  Navigation](https://workday.github.io/canvas-kit/?path=/docs/guides-accessibility-examples-side-panel-navigation--docs).
  Content is **`as="ul"`**; omit **`headingLevel`**.
...
default — the Basic Example code uses `h4` only as a sample. Focus stays on
**`Expandable.Target`** when the disclosure opens or closes (see **Keyboard**).

**Accordion** _(conditional)_:

Use this composition when several related Expandables form an accordion. Default
**`Expandable.Content`** is a `<div>`, not a landmark. For each item, assign a unique `id` to
**`Expandable.Title`** and render **`Expandable.Content as="section"`** with `aria-labelledby`
pointing at that title `id`. Ask **`headingLevel`** when the headers title body content. Do not also
set **`as="ul"`** on that Content. See [Hoisted Model](#hoisted-model).
...
**Navigation list** _(conditional)_:

When the disclosed content is a list of links, render **`Expandable.Content as="ul"`** and omit
**`headingLevel`**. Do not also set **`as="section"`** on that Content. See
[Side Panel with Navigation](https://workday.github.io/canvas-kit/?path=/docs/guides-accessibility-examples-side-panel-navigation--docs).
