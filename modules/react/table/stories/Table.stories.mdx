...
The `InputAlert` and `InputError` states should be used if the error is on the input-level rather
than the entire row. They are identical to the `Alert` and `Error` states, respectively, except they
lack a darkened border around the row.
