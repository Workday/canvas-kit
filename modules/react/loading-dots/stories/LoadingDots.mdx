...
import {DarkBackgrounds} from './examples/DarkBackgrounds';
...
<ExampleCodeBlock code={DarkBackgrounds} />
...
Loading Dots exposes the following props:

- `loadingDotsColors`: Applies backgroundColor to loading dots, intended for use with the circle variant design on grey/dark/image-based backgrounds. Defaults to `system.color.bg.alt.strong`.
- `animationDurationMs`: Duration of the loading animation. Defaults to `40ms`.

Other undocumented props are spread to its outermost element.
