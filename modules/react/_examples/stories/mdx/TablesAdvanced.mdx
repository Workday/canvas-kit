...
popup elements at the bottom of the browser's DOM and presents 2 key challenges for accessibility:
...
- To address the reading order of content, we set the `aria-owns` property onto the parent
  `<Table.Header>` component (`<th>` element) with 2 unique `id` values. The first `id` refers to
  the `<Popup.Target>` button and the second refers to the `<Popup.Card>` container element. This
  ...
  header buttons and the popup content logically in order even though they are not siblings in the
  DOM.
...
- The `aria-sort` property has been added to each of the `<Table.Header>` components (`<th>`
