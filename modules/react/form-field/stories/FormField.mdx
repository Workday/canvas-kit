...
Some composite controls such as `MultiSelect`, have additional sub-elements that also need to be linked to the `FormField.Label`. In the example belwo, the FormField.Label is applied to both the `input[role=combobox]` element and the `ul[role=listbox]` element so that screen reader knows that the label for both is the "States you've lived in".
