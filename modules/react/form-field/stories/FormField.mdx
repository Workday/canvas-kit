...
To apply the Sana Canvas theme to Form Field, apply the `data-theme="sana-canvas"` attribute to your
component or to any ancestor element (such as `body`) if you wish to scope the theme to a larger
area.

This example applies `data-theme="sana-canvas"` directly to the Form Field.
