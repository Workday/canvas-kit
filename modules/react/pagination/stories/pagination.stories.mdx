...
`Pagination` is a [compound component](/docs/guides-compound-components--page) that allows users to
navigate between pages in a range.
...
access to its `state` and `events` outside of the `Pagination` component.
