...
icons. `InputGroup.Start` and `InputGroup.End` are used to position elements at the start and end of
...
`InputGroup.Start` and `InputGroup.End` subcomponents can handle any child elements, but are built
for icons. The default width is `40px`, which is perfect for icons. If you need to use something
else, be sure to set the `width` property of `InputGroup.Start` or `InputGroup.End` to match the
intended width of the element.
