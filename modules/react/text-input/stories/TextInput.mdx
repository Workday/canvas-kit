...
> contrast, making them unreliable for screen readers. Use placeholders only for short format
> examples (e.g., "name@example.com"), and place detailed instructions or guidance in
> `FormField.Hint` instead of the placeholder.
...
Set the `error` prop of the wrapping Form Field to `"caution"` or `"error"` to set the Text Input to
...
`TextInput` should be used with [Form Field](?path=/docs/components-inputs-form-field--docs) to
...
While `TextInput` renders `<input type="text">`, consider using specialized input types for better
mobile keyboard support:

- `type="email"` for email addresses (shows @ key on mobile keyboards)
- `type="tel"` for phone numbers (shows numeric keypad on mobile)
- `type="url"` for web addresses (shows .com key on mobile keyboards)
- `type="search"` for search inputs (shows search icon and enables search-specific behaviors)
