...
> in this example need no extra attributes. If an icon conveys meaning beyond the label text, provide
> that meaning as text for screen readers.
...
matches the DOM reading sequence and ensures the label's `htmlFor` targets the `<input>` before
