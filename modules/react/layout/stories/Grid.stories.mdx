...
<Meta title="Components/Containers/Grid" component={Grid} />
...
`Grid` exposes [grid container style props](/docs/features-style-props--grid-example) and `Box`
style props.

### Grid.Item

`Grid.Item` is a subcomponent of `Grid`. It is a `Box` component under the hood and exposes
[grid item style props](/docs/features-style-props--grid-item-example) that map to CSS Grid Item
