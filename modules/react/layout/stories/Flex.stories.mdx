...
<Meta title="Components/Containers/Flex" component={Flex} />
...
`Flex` exposes [flex container style props](/docs/features-style-props--flex-example) and `Box`
style props.

### Flex.Item

`Flex.Item` is a subcomponent of `Flex`. It is a `Box` component under the hood and exposes
[flex item style props](/docs/features-style-props--flex-item-example) that map to CSS Flexbox Item
...
`Flex.Item` exposes [flex item style props](/docs/features-style-props--flex-item-example) and `Box`
style props.
