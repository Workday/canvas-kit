...
import SegmentedControlStoriesMeta from './SegmentedControl.stories';

<Meta of={SegmentedControlStoriesMeta} />
...
- Providing your own `aria-label` string to `SegmentedControl.List` is recommended for describing
...
- icon-only items must have an accessible name, supplied by visible text, aria-label, or a Tooltip
  in label mode.
...
<Specifications file="./cypress/component/SegmentedControl.spec.tsx" name="SegmentedControl" />
