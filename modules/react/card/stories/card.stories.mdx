...
<NoticeCard headingText="Careful!">

Changing the `Card` container to certain semantic elements will put accessibility at risk. For example,
using the `as` prop to change the cards to buttons will flatten the content in the card. Headings, calls to action, etc.
will not function as expected for users with disabilities. Semantic container elements like `<section>``, or using `<li>`
grouped together in a common `<ul>` can be a useful way to elevate the accessibility of your design.
