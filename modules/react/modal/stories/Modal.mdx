...
>**Note: We recommend the primary action button (i.e. "Acknowledge", "Submit", "Accept", etc.) to be "end-aligned".
> This would be "right-aligned" for a LTR (left-to-right) language and "left-algined" for a RTL (right-to-left) language.**

>**Using "start" and "end" reduces the confusion for RTL (right-to-left) or LTR (left-to-right) languages.**
