...
does **not** add the sibling **`aria-owns`** pattern used to remap reading order for portaled
non-modal dialogs. Focus moves into the modal when it opens, and sibling hiding reduces exposure to
content behind the overlay. For portals, reading order, and related tradeoffs, see
