...
Set the `error` prop of the wrapping Form Field to `"caution"` or `"error"` to set the Text Area to
...
The `error` prop may be applied directly to the Text Area with a value of
`"TextArea.ErrorType.Caution"` or `TextArea.ErrorType.Error` if Form Field is not being used.
