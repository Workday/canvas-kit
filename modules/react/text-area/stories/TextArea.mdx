...
Use `TextArea` when users need to enter multiple lines or paragraphs of text. For single-line
values (names, emails, short answers), use
[TextInput](/components/inputs/text-input/) instead.
...
- **`FormField.Input`**: Applies `useFormFieldInput` to wire `id` (`input-{id}`), `aria-labelledby`
  (`label-{id}`), `aria-describedby` (`hint-{id}` when an `id` exists), `required` when
  `isRequired`, `aria-invalid="true"` when `error="error"`, and passes `error` for visual styling.
  Caution (`error="caution"`) does **not** set `aria-invalid`.
...
- Clicking **`FormField.Label`**: Moves focus to the associated `<textarea>` (native `<label>`
  behavior).
- Inside the text area: arrow keys, <kbd>Home</kbd>, <kbd>End</kbd>, and standard text-editing
  shortcuts behave as the browser and operating system provide for `<textarea>` elements.
...
For a simple text area field without hint text, the DOM looks like:
...
`FormField.Input as={TextArea}`, omit `isHidden`, keep default `resize: both`, and omit a custom
`id` unless testing or composition requires it.
...
- Manually set `id`, `htmlFor`, `aria-labelledby`, `aria-describedby`, `aria-invalid`, or `required`
  on **`FormField.Input`** or **`TextArea`** — `FormField` subcomponents wire these automatically
  via `useFormFieldInput` and related hooks.
