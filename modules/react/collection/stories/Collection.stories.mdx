...
import {RegisteringItems} from './examples/RegisteringItems';
...
There are two ways to register items via the `items` prop or model config.

1. Array of Objects
   - Use this format when your data has additional information.
2. Array of Strings
   - Use this format when your data is a simple list of options.

If you have an array of `strings`, the collection system will handle registering items. Each string
value is used as the unique identifer.

```tsx
<ListBox items={['Pizza', 'Chocolate', 'Cheeseburgers']}>
  {item => {
    return <ListBox.Item>{item}</ListBox.Item>;
  }}
</ListBox>
```

If you have an array of `objects`, the collection system needs an `id` to ensure items are
registered and searchable. Any additional information like icons or if the item is disabled can
exist on the object and be used in the render function.
...
<ExampleCodeBlock code={RegisteringItems} />
