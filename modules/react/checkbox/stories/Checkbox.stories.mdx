...
[Form Field](/docs/components-inputs-form-field-react--default) since it includes a `<label>` with a
...
explanation using the `hintText` prop to set the Checkbox to the Alert state. You may omit the
`label` prop on the Form Field.

<ExampleCodeBlock code={Alert} />

The `error` prop may also be applied directly to the Checkbox with a value of
`Checkbox.ErrorType.Alert` if Form Field is not being used.

#### Error

Set the `error` prop of the wrapping Form Field to `FormField.ErrorType.Error` and provide an
explanation using the `hintText` prop to set the Checkbox to the Error state. You may omit the
`label` prop on the Form Field.

<ExampleCodeBlock code={Error} />

The `error` prop may also be applied directly to the Checkbox with a value of
`Checkbox.ErrorType.Error` if Form Field is not being used.
