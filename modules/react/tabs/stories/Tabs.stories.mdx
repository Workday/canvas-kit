...
like so allows direct access to its state and events outside of the `Tabs` component. Create the
...
Creates a `TabsModel` using the provided configuration object.

Use `useTabsModel` if you wish to create a `TabsModel` rather than relying on the `Tabs` component
to create and use its own model internally. Doing so allows you to access the model's state and
events directly.
...
`useTabsModel` accepts a configuration object with the following properties.

<ArgsTable of={TabsModelConfigComponent} />

#### Return Value

`useTabsModel` returns a `TabsModel` with `state` and `events` properties.

##### State

`state` is an object with the following properties which describe the current snapshot in time of
the component.
...
`events` is an object with the following properties which describe events that can be sent to the
model.
