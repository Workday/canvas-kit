...
import {InlinePopup} from './examples/InlinePopup';
...
Dialog-style popups. Focus redirection tries to treat the Popup as if it were inline to the
document. Tabbing out of the Popup will close the Popup and move focus to the next appropriate
element.
...
> hierarchy of the accessibility tree to address the reading order problem. For another cool
> example, check out the `<Table.Header>` components with `aria-owns` in the
> [Table with Filterable Column Headers](?path=/docs/guides-accessibility-examples-advanced-tables--docs#filterable-column-headers).

### Inline Popups With `portal={false}`

By default, the popup content is rendered in a portal at the end of `document.body`. Set
`portal={false}` on `Popup.Popper` to render the popup inline in the DOM next to its target. Inline
popups stay within their parent container and are constrained by ancestor overflow (e.g.
`overflow: hidden` or scroll containers), which can be useful for scrollable regions or when you
need the popup to be part of the document flow.

<ExampleCodeBlock code={InlinePopup} />
