...
Each `Popup` subcomponent optionally takes a `model`. Sometimes you may want to render an open Popup
because visibility is controlled elsewhere. Behaviors require a model, so you can initialize the
model visibility set to `true`. This may be required for legacy uses or where the entire `Popup`
