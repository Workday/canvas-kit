...
When registering items in an array of objects, it's common to have the text displayed to the user be
different than an id. In this example, `serverId` and `label` properties need to be re mapped to
...
- `getTextValue`: Optional function to return the text representation of an item. If not provided,
  ...
  **Note: If your array of objects has a different property for `text`, like `label`, use this
  ...
  ```tsx
  const options = [{label: 'Pizza', id: 'pizza-1'}, {label: 'CheeseBurger', id: 'cheese-burger'}]
