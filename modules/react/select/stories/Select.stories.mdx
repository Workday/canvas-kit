...
- Pass the value to the input in the follow way:
  `<Select.Input inputStartIcon={currentItem.value.icon} id="form-field-input-id" />`

**Notes: The `<Select.Input/>` will only render an icon when there is a selected item.**
...
It's important to set up the object in your items array in a way for the collection system to
understand.
