...
[**MultiSelect**](/docs/preview-multiselect--docs)
...
pattern. Canvas Kit sets `aria-autocomplete="list"` (the APG example uses `"none"`) and still does
not insert typed characters into the input — do not override it to `"none"`.
...
- **REQUIRED:** visible label, **`FormField.Input as={Select.Input}`**, popup list composition
  (**`Select.Popper`**, **`Select.Card`**, **`Select.List`**, **`Select.Item`**), **`items`**. Default
  list rendering is `{item => <Select.Item>{item}</Select.Item>}` (or `item.text` / **`getTextValue`**).
- **CONDITIONAL:** disabled combobox, disabled options (`nonInteractiveIds` + `aria-disabled`),
  grouped list items (**`items`** plus static **`Menu.Group`** / **`data-id`** children — do not omit
  **`items`**), option icons, custom placeholder, programmatic focus via `ref`, **`getId`** /
...
- **Duplicating combobox ARIA**: Do not set `role`, `aria-haspopup`, `aria-expanded`,
  `aria-autocomplete` (including `"none"`), `aria-controls`, `aria-activedescendant`, `autoComplete`,
  or listbox `role` / `id` / `aria-labelledby` / `aria-label` by hand — **`Select.Input`** and
  **`Select.List`** wire these. **`FormField.Label`** names the combobox; do not add a name on the
  listbox.
...
- **`Combobox` or `TextInput` when the value must be one list option**: Use **`Select`**. Use
  [**MultiSelect**](/docs/preview-multiselect--docs) when
...
- **`data-id` on dynamic items**: Do not set **`data-id`** on `{item => <Select.Item>}` — the
  ...
  (see **Static item ids**).
- **Grouped list without `items` or without static children**: Do not omit **`items`** when grouping,
  and do not group with `{item => <Select.Item>}` alone — use **`Menu.Group`** and **`data-id`** (see
  **Grouped list items**).
