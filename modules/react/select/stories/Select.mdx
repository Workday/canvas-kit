...
| Item identity                            | Pass **`items`**. When objects are not `{id, text}`, set **`getId`** and/or **`getTextValue`**. When rendering object items, set **`data-id`** on **`Select.Item`** to the same id used by the collection                                                         |
...
- **CONDITIONAL:** disabled combobox, disabled options (`nonInteractiveIds` + `aria-disabled`),
  ...
  `getTextValue` / `data-id` when item shape is not `{id, text}`. See
