...
import {NonSkeletonChildren} from './examples/NonSkeletonChildren';
...
before they have displayed on the page. Appearing as a blank version of the page, it mitigates focus
on the loading process and improves the user’s perceived load time.
...
Skeleton includes a container component (`Skeleton`) as well as a number of subcomponents
(`Skeleton.Header`, `Skeleton.Text`, and `Skeleton.Shape`) which can be used as placeholders for
different types of content.

Here's an example of how Skeleton components can be combined to create a loading placeholder for
Card content. Press the **Simulate Loading** button to simulate the loading of the content
(customize the loading time using the **Load Time** field), or check the **Loading** check box to
force the Skeleton to display.
...
### Non-Skeleton Children

Note that non-Skeleton components can be included as children of the `Skeleton` container component
if you'd like to customize your Skeleton further.

<ExampleCodeBlock code={NonSkeletonChildren} />
