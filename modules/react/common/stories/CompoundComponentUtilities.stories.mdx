...
`createComponent` is a great factory function that creates components to be exported. It enforces
React ref forwarding, `as` prop, display name, and sub-components, and handles proper typing without
much boiler plate. The return type is `Component<element, Props>` which looks like
