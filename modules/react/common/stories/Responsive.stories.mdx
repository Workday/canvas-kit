...
Now, with viewport-based responsive styling (media queries), the user's viewport (user's visible
area of a web page) is measured and styles are applied based on the size of that viewport. We will
do the same with Container Based Responsive Styling but, we will just be measuring the size of the
...
In the example above, we see that there is a `maxWidth` set on the Box component of `1000`. This
will set the maximum width of that element at `1000px`. What we can do with Container Based
Responsive Styling is measure when that container's width reaches certain breakpoints
