...
layout that can respond to the device's viewport size.

You can read more about the history of
[Responsive Design](https://developer.mozilla.org/en-US/docs/Learn/CSS/CSS_layout/Responsive_Design).

## Container and Viewport Based Responsive Styling

We will take a look at Container-based and Viewport-based Responsive Styling, what they are and
which to use.

### Container Based Responsive Styling

With viewport-based responsive styling (media queries), the user's viewport (user's visible area of
a web page) is measured and styles are applied based on the size of that viewport. We will do the
same with Container Based Responsive Styling but, we will just be measuring the size of the
container the element is in instead of the user's viewport.
...
change to match the size shown in the dropdown. This is just an example of how container-based
styles work and will be explained more in depth below.

<ExampleCodeBlock code={ResponsiveContainer} />

### Applying Container-Based Responsive Styling

For container-based responsive styling, we have utilities that will allow you to apply responsive
styles at those breakpoints. There are four utilities/hooks:
`useResizeObserver | useResponsiveContainerStyles`

`useResizeObserver` - This is a hook that will provide the width of the container. This can be used
to measure container size as well.
...
In the example below, we are importing these utilities:
`useResizeObserver | useResponsiveContainerStyles`
...
The viewport is the user's visible area of a web page and it varies with screen size.
...
In the example below, viewport-based styles are being applied by the size of the user's viewport.
This is by using the breakpoint utilities (`up(), down(), between() and only()`). At the top of the
code you will see `up() and down()` being used inside of the `StyledHeading` component. These
utilities will be explained more in depth below the example. The size of the browser will have to be
adjusted to see the changes take place on the UI Layout.
