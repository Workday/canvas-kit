...
`-surface-selected`) uses Sana's neutral ramp (`neutralA900`/`neutralA100`), the same values as
classic Canvas — selection styling doesn't change under Sana, whether or not
