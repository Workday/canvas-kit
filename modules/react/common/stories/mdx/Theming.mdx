...
Prior to v14, the `usePopupStack` would grab the theme passed to the `CanvasProvider` and generate a
`className` that would be added to Modals and Dialogs, forwarding the theme set at the `theme` prop
level by the `CanvasProvider`. This would create a cascade barrier for any CSS variables defined at
the root since the `className` had higher specificity.

This has since been removed to ensure global theming works as intended, meaning CSS variables
defined at the root level will cascade through to Modals and Dialogs.
...
const myTheme = createStyles({
  [brand.primary.base]: 'blue'
})

<CanvasProvider className={myTheme}>
  <Modal>//... rest of modal code</Modal>
</CanvasProvider>
```
