...
<CanvasProvider theme={{canvas: {palette: {primary: base.magenta600}}}}>
   <App/>
</CanvasProvider>
```
...
- Runtime theme generation using OKLCH color space
...
### Method 2: Component-Level CSS Variables
...
const MyComponent = () => (
  <div className={greenTheme}>
    <PrimaryButton>Success Action</PrimaryButton>
  </div>
);
```
...
Create dedicated theme files for different contexts:

```
styles/
  themes/
    base-theme.css          // Default theme
    dark-theme.css          // Dark mode overrides
    brand-variations/
      marketing-theme.css   // Marketing site theme
      admin-theme.css       // Admin interface theme
```

### 3. Document Color Relationships
When creating custom themes, document the color relationships:

```css
:root {
  /* Primary brand colors - maintain accessibility ratios */
  --cnvs-brand-primary-base: var(--cnvs-base-palette-blue-600);    /* 4.5:1 contrast */
  --cnvs-brand-primary-light: var(--cnvs-base-palette-blue-200);   /* Background use */
  --cnvs-brand-primary-dark: var(--cnvs-base-palette-blue-700);    /* Hover state */
}
```
