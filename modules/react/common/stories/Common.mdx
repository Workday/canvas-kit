...
The `Base` component is a low level component that supports the [`cs`](https://workday.github.io/canvas-kit/?path=/docs/styling-basics--docs#cs-prop) prop and style merging via [`handleCsProp`](https://workday.github.io/canvas-kit/?path=/docs/styling-merging-styles--docs#handlecsprop).
As we transition away from runtime styles, you should use `Base` to style an element if a Canvas Kit component doesn't fit your needs.
...
> `Box` and `Flex` are great. Why are you moving away from them?
