...
export const DisclosureExample = createContainer('div')({
  modelHook: useDisclosureModel,
})(({}, _, model) => {
  return (
    <div>
      <button onClick={() => model.events.show()}>Open</button>
      {model.state.visibility === 'visible' ? <div>Show content</div> : null}
    </div>
  );
});
