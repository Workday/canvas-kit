...
Custom targets must forward both to a **keyboard-focusable** element (prefer a native **`<button>`**
or **`as={SecondaryButton}`**). Use **`React.forwardRef`** when the dialog may open programmatically
before the user activates the target—without **`forwardRef`**, the dialog can open at the top-left
of the viewport instead of anchored to the target.

**Programmatic open without `Dialog.Target`:**
...
const model = useDialogModel({returnFocusRef: openButtonRef});
...
**Custom target with `forwardRef`** _(when the dialog may open before the user clicks the target)_:
...
const MyTarget = React.forwardRef<HTMLButtonElement, MyTargetProps>(
  ({label, ...props}, ref) => (
...
<Dialog>
  <Dialog.Target as={MyTarget} label="Open" />
  <Dialog.Popper>{/* ... */}</Dialog.Popper>
</Dialog>;
```
...
| Keyboard-operable trigger                                     | See **Custom targets and programmatic open** above.                                                                                                                                                                                                                                                                                                                                                                                                                                    |
...
- **CONDITIONAL:** **`initialFocusRef`**, **`returnFocusRef`**, **`aria-describedby`**, **`aria-expanded`**
  / **`aria-haspopup`**, **`forwardRef`** on custom **`Dialog.Target`**
