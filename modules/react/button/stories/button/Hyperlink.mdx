...
import {NeutralLink} from './examples/NeutralHyperlink';
...
`Hyperlink`s also have a `neutral` variant. This is an underlined dark gray alternate to the default
blue hyperlink. Unlike `standalone`, it keeps the underline. This variant is meant for use on
colored backgrounds.

This variant can also be used with the `ExternalHyperlink` component.

<ExampleCodeBlock code={NeutralLink} />
