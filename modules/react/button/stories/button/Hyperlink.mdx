...
This removes the underline. By default, `type` is `"inline"` and links are underlined.

This prop can also be used with the `ExternalHyperlink` component, and can be combined with
`variant` (for example, `variant="secondary"` `type="standalone"`).
...
hyperlink. Combine this with `type="standalone"` to remove its underline. This variant is meant for
