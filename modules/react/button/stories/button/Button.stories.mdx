...
If your link should open in a new tab or window add the attribute `target='_blank'`. For added
security you should also include `rel='noreferrer'`. It is also typical to let users know about the
behavior by adding an icon, e.g. `extLinkIcon`. Adding an `img` role and and `aria-label` to the
icon will make it accessable.
