...
In a basic example of an `ActionBar` there are two buttons. The primary (or delete) action button
should be used only once and left aligned if content is left to right, followed by secondary
buttons. Tertiary buttons should not be used in the Action Bar.
