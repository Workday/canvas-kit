...
contains primary and secondary actions related to a page or task.
...
In a basic example of an `ActionBar` there is two buttons. The primary action button should be used
...
```tsx
<ActionBar items={[]}>{/* Child components */}</Tabs>
```
