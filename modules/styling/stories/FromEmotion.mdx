import * as FromEmotionStories from './FromEmotion.stories';

<Meta of={FromEmotionStories} />
...
<ExampleCodeBlock code={FromEmotionStories.EmotionButton} />
...
<button
  ...
  style={{'--background-button-color': 'orange'}}
>
  Orange Large
</button>
```
...
```html
<button class="css-puxv12 css-puxv16" style="--background-button-color: orange;">
  Orange Large
</button>
```
...
<ExampleCodeBlock code={FromEmotionStories.ManualStylesButton} />
...
myVars.color // something like `--{hash}-color`

// the function knows what keys are allowed
myVars({color: 'red'}) // {'--{hash}-color': 'red'}

// in a component
<div style={myVars({color: 'red'})} /> // <div style="--{hash}-color: red;">
```

<ExampleCodeBlock code={FromEmotionStories.StylingButton} />
