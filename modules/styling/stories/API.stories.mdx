...
For a transition period, we're opting for backwards compatibility. If style props are present in a
component that supports both the `cs` prop and style props, Emotion's style merging will be invoked
to make sure the following style precedence:

```
CSS Prop > Styled Component > createStyles + cs prop > Style props
```

This will mean that any `css` prop or use of `styled` within the component tree _per element_ will
win over `createStyles`. For example:
...
const Component1 = props => {
  return <StylingBox {...props} cs={[styles1, props.cs]} />;
};
...
const Component3 = props => {
  return <Component2 {...props} cs={[styles2, props.cs]} />;
};
```

`styled` components and the `css` prop work by merging all properties in the order by their class
names. The expectation might be that the final padding should be `12px` since `Component3` is the
final componment to set the padding. But CSS merges by
[specificity](https://developer.mozilla.org/en-US/docs/Web/CSS/Specificity). `createStyles`,
`styled` and the `css` prop all create a single CSS class selector which is a specificity of
`(0, 1, 0)`. If the specificity of all selectors is the same, the browser will merge in the order
that selectors are inserted into the document's
[StyleSheetList](https://developer.mozilla.org/en-US/docs/Web/API/StyleSheetList). `createStyles`
injects styles into the `StyleSheetList` immediately, while the runtime APIs of `styled` and the
`css` prop inject styles when the component is rendered. The actual padding will be `8px` The
resulting DOM is the following:
...
<div class="component1 component3 component2"></div>
```
...
Style props must be evaluated at runtime and use the `styled` component. In order to ensure style
props win over a component's local styling that uses `createStyles`, a `styled` component is used to
ensure Emotion merges styles together.

If we modify the top example to add a style prop, the class names will be removed and a new class
name will be applied with properties merged according to Emotion's runtime merging rules:
...
export default () => {
  return <Component3 backgroundColor="transparent" />;
};
```

The precense of a style prop forces the `StylingBox` to use a `styled` component instead of a
regular component which forces Emotion to merge styles together.
...
<div class="css-somehash"></div>
```

`StylingBox` adds `className` to the end by merging like this: `[cs, className]`. This is why the
`component2` class name is at the end of the class list of the previous example. This instructs the
merge order of Emotion. We did this sepecifically to ensure the same merge behavior in both cases.
