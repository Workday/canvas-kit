...
<Meta title="Styling/Reference" />

# Canvas Kit Styling Reference
...
<SymbolDoc headingStart={2} name="createStyles" />

<SymbolDoc headingStart={2} name="createStencil" />

<SymbolDoc headingStart={2} name="handleCsProp" />

<SymbolDoc headingStart={2} name="mergeStyles" />
...
export const LoadingDot = createComponent('div')({
  ...
  Component: ({ ...elemProps }: CSProps, ref, Element) => {
    return (
      <Element
        ref={ref}
        {...handleCsProp(elemProps, loadingStencil())}
      ></Element>
    );
  },
});
