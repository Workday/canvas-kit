...
<Meta title="Styling/Stencils" />

# Stencils

Stencils are a reusable function that returns `style` and `className` props in an object. A Stencil
should apply to a single element. If your component has nested elements, you can youse `parts` to targer those elements in the Stencil. If your component is a compound
