...
describe('CSS color-mix Functions', () => {
  ...
  describe('hover', () => {
    it('should return color-mix of hover state using accent tokens', () => {
      ...
      expect(expected).toBe(
        `color-mix(in srgb, var(${color}, var(${fallback})) , var(--cnvs-sys-color-${colorType}-overlay-hover, black) calc(var(--cnvs-sys-opacity-${colorType}-hover, 0.18) * 100%))`
      );
    });
    it('should return color-mix of hover state using surface tokens', () => {
      ...
      expect(expected).toBe(
        `color-mix(in srgb, var(${color}, var(${fallback})) , var(--cnvs-sys-color-${colorType}-overlay-hover-default, black) calc(var(--cnvs-sys-opacity-${colorType}-hover, 0.08) * 100%))`
      );
    });
  });
  describe('pressed', () => {
    it('should return color-mix of pressed state using accent tokens', () => {
      ...
      expect(expected).toBe(
        `color-mix(in srgb, var(${color}, var(${fallback})) , var(--cnvs-sys-color-${colorType}-overlay-pressed, black) calc(var(--cnvs-sys-opacity-${colorType}-pressed, 0.36) * 100%))`
      );
    });
    it('should return color-mix of pressed state using surface tokens', () => {
      ...
      expect(expected).toBe(
        `color-mix(in srgb, var(${color}, var(${fallback})) , var(--cnvs-sys-color-${colorType}-overlay-pressed-default, black) calc(var(--cnvs-sys-opacity-${colorType}-pressed, 0.18) * 100%))`
      );
    });
  });
});
