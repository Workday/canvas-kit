...
describe('CSS color-mix Functions', () => {
  ...
  describe('hover', () => {
    it('should return color-mix of hover state using accent tokens', () => {
      ...
      expect(expected).toBe(
        `color-mix(in srgb, var(${color}, var(${fallback})) , var(--cnvs-sys-color-${colorType}-overlay-mixin, oklch(0 0 0 / 1)) calc(var(--cnvs-sys-opacity-${colorType}-hover, 0) * 100%))`
      );
    });
    it('should return color-mix of hover state using surface tokens', () => {
      ...
      expect(expected).toBe(
        `color-mix(in srgb, var(${color}, var(${fallback})) , var(--cnvs-sys-color-${colorType}-overlay-mixin, oklch(0.629 0.0281 255.62 / 1)) calc(var(--cnvs-sys-opacity-${colorType}-hover, 0) * 100%))`
      );
    });
  });
  describe('pressed', () => {
