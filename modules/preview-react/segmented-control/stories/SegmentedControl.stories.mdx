...
specified for size (as `medium`) and orientation (as `horizontal`). All buttons should have the same
type amd only one of them can be active at a time.
...
`SegmentedControl` can be used with diferent buttons: text only, icon left or icon only.
...
`SegmentedControl` icon only buttons can be placed horizontally or vertically.
...
``SegmentedControl`can be disabled by adding`disabled` prop to the whole container. It will also
disable all buttons.
