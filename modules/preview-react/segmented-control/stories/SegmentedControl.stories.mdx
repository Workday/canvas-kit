...
a linear group of two, three or four buttons allowing select specific value. This is a controlled
component.
...
In a basic example of an `SegmentedControl` there are four icon buttons in group. Each button is
...
`SegmentedControl.List` which contains a `SegmentedControl.Item`.
