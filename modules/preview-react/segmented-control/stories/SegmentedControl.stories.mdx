...
a linear group of multiple buttons allowing the selection of a specific value. We strongly encourage
to use no more than 4 buttons.
...
Note that `SegmentedControl.Item` accepts an optional `index` prop if you want to specify the
position of the item. Items are appended to the end by default.
