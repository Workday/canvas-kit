...
import {RadioGroup2} from '@workday/canvas-kit-preview-react/radio';
...
<Meta title="Preview/Radio" component={RadioGroup2} />
...
Additionally, you can provide an `initialValue` to the `RadioGroup` element so that a value is pre
selected from the group.

To tie it all together, provide an `onChange` to `RadioGroup` element that sets the selected value
on [state](https://react.dev/learn/state-a-components-memory).

Also, each `RadioGroup.Radio` must have a unique `value`. This is used to determine which radio has
been selected.
...
Use alert when a selection is still valid but there might be addiotional information that might be
important to show.
