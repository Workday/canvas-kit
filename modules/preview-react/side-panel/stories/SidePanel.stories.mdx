...
// Prop Tables
import {UseSidePanel, Panel, Label, Control} from './examples/PropTables';
...
of four `SidePanelTransitionStates`, `expanding`, `expanded`, `collapsing`, and `collapsed`. Below is an example:
...
const {expanded, setExpaned, panelProps, labelProps, controlProps} = useSidePanel({
