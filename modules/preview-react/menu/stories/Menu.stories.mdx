...
If you're using a menu button, you'll need to add your own keyboard shortcuts, aria attributes, and
focus management. Note that if you're using [Popup](/components/popups/popup/), you do **not** need
to use the `useCloseOnEscape` or `useInitialFocus` hooks since `Menu` already handles those
behaviors internally (`isOpen` must be set properly in order for the `Menu` to receive focus when it
appears).
