...
`Menu` renders a styled `<ul>` element with a role of `menu` and follows the [Active Menu pattern](https://www.w3.org/TR/wai-aria-practices/examples/menu-button/menu-button-actions-active-descendant.html) using `aria-activedescendant`.
...
`MenuItem` renders a `<li>` element with the correct attributes to ensure it is accessible. If you choose to implement your own custom menu items, be sure to use semantic `<li>` elements with the following attributes:
