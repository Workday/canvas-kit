...
<Meta title="Preview/Status Indicator" component={StatusIndicator} />
...
of `StatusIndicator` via [style props](/docs/features-style-props--background-example).
...
`StatusIndicator` is a container component which renders an
[`HStack`](/components/layout/stack/#hstack) under the hood to apply spacing evenly between its
children. It has a default maximum width of `200px`.
...
`StatusIndicator.Label` renders [`Text`](/components/text/text/) under the hood. It will apply an
ellipsis if its contents exceed the component's maximum width.
...
`StatusIndicator.Icon` renders [`SystemIcon`](/assets/system-icons/) under the hood. It's used as a
decorative element to visually support the `StatusIndicator.Label` text.
