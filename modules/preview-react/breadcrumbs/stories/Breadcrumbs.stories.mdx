...
Accessibility is built into Breadcrumbs components in a way that allows you to create an inclusive
experience with little additional configuration. The only ARIA labels you'll need to add are the
`aria-label` for `Breadcrumbs.Nav` and the `buttonAriaLabel` for `Breadcrumbs.CollapsibleList`. And
all links require `href`s to be properly identified. All other accessible attributes are baked into
the components.
...
Breadcrumbs has bidrectional support out of the box. That means outside of setting the content
...
managing the length of link text. By default, the `maxWidth` is set to `350px`, however you can
adjust this as needed. Note that tooltips will only render when text is truncated. Here's an example
with the `maxWidth` set to `150px`.
...
feature for managing the length of link text. By default, the `maxWidth` is set to `350px`, however
you can adjust this as needed. Normally, this is a non-focusable element. But when truncated, it
sets the `tabIndex` to `0` to enable the tooltip to appear on keyboard focus. Note that tooltips
will only render when text is truncated. Here's an example with the `maxWidth` set to `100px`.
