...
> **_NOTE:_** By default, the Pill's width will max out at `200px` and the text will overflow into a
> tooltip. This max width can be changed by providing a `maxWidth` prop on the pill.
...
> **_NOTE:_** The `Pill.Icon` icon will default to rendering a `plusIcon`. You can change this by
> providing a `icon` prop to `Pill.Icon`
...
> **_NOTE:_** The default icon for `Pill.IconButton` is `xSmallIcon` but this can also be
> overwritten by passing an `icon` prop to `Pill.IconButton`
...
If you give this component an `onClick` it will render as an interactive pill where sub components
are acceptable
...
This component renders an avatar but you can still pass it avatar props

<ArgsTable of={Pill.Avatar} />

### Pill Count

This component renders the count. The `child` element is what gets rendered as the count.

<ArgsTable of={Pill.Count} />

### Pill Icon

This component renders an `icon`. This component is used when an `onClick` is provided.

<ArgsTable of={Pill.Icon} />

### Pill IconButton

This component renders a `icon button`. This is used for `removable` Pills so that the click target
and focus area is soley the icon.

<ArgsTable of={Pill.IconButton} />

### Pill Label

This component renders a `span` that automatically handles overflow and rendering a tooltip. There's
no need to use this component directly since the overflow is handle for you automatically.
