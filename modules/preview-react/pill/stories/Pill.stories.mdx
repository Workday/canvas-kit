...
If you give the component an `onClick` and an `onDelete` this is not a valid configuration and
nothing will render. You should see a warning in the console stating to provide one or the other.
...
By default this component render a `plusIcon` but it can be overriden.
...
focus area is solely the icon. By default, this component renders a `xSmallIcon` but can be
overriden.
