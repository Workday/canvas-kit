...
const transform: Transform = (file, api) => {
  ...
  const existingSwitchImports = root.find(j.ImportDeclaration, {
    source: {value: '@workday/canvas-kit-preview-react/switch'},
  });
  ...
  if (existingSwitchImports.length) {
    existingSwitchImports.forEach(nodePath => {
    ...
    if (foundImport.length) {
      foundImport[0].insertBefore(
        j.importDeclaration(
          informationHighlightSpecifiers.map(mapToSpecifiers),
          j.stringLiteral('@workday/canvas-kit-preview-react/switch')
        )
      );
    }
  }
  ...
  foundImport.forEach(importPath => {
    if (
      ...
      importPath.value.source.value === '@workday/canvas-kit-react/switch'
    ) {
      importPath.prune();
    }
  });
