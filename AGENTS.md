...
## MCP catalog (modules/mcp)

`modules/mcp` ships an MCP server whose component/token/icon catalogs are generated by
[`build/generate-indexes.ts`](modules/mcp/build/generate-indexes.ts) into committed JSON files
(`lib/component-index.json`, `lib/token-index.json`, `lib/icon-index.json`). If you're touching
anything under `modules/mcp`:

- Check the expected-version constants at the top of `generate-indexes.ts`
  (`TOKEN_PACKAGE_VERSION`, `PRODUCTION_ICON_PACKAGE_VERSION`, `VISION_ICON_PACKAGE_VERSION`)
  against the actual latest published versions of `@workday/canvas-tokens-web` and
  `@workday/canvas-system-icons-web` on npm. These drift silently — nothing fails CI if they're
  stale, you just get a console warning at generation time.
- If you bump a constant, regenerate the affected catalog (`yarn build:indexes` from
  `modules/mcp`) rather than hand-editing every field in the JSON — hand-edit only the specific
  `meta.sources[].version` string if a full regen would introduce unrelated diff noise (e.g. from
  a locally stale `canvasKitVersion` or toolchain drift).
- `PRODUCTION_ICON_PACKAGE_VERSION` intentionally tracks the legacy
  `@workday/canvas-system-icons-web-v4` package (used only to check whether an icon already ships
  in production Canvas Kit) — don't bump it in lockstep with `VISION_ICON_PACKAGE_VERSION`.
- Run `yarn test:catalog` (from `modules/mcp`) after any catalog or generator change.
